...
cnoremap <Leader>`d <C-R>=strftime("%Y-%m-%d")<CR>
inoremap <Leader>`D <C-R>=system("echo -n $(date -Iseconds)")<CR>
cnoremap <Leader>`D <C-R>=system("echo -n $(date -Iseconds)")<CR>
inoremap <Leader>`t <C-R>=strftime("%T")<CR>
cnoremap <Leader>`t <C-R>=strftime("%T")<CR>
inoremap <Leader>`u <C-R>=system("echo -n $(date -Idate -u)")<CR>
cnoremap <Leader>`u <C-R>=system("echo -n $(date -Idate -u)")<CR>
inoremap <Leader>`U <C-R>=system("echo -n $(date -Iseconds -u \| sed 's/+00:00/Z/')")<CR>
cnoremap <Leader>`U <C-R>=system("echo -n $(date -Iseconds -u \| sed 's/+00:00/Z/')")<CR>
